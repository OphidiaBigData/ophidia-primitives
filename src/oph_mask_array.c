...
|------------------------------------------------------------------*/
my_bool oph_mask_array_init(UDF_INIT *initid, UDF_ARGS *args, char *message)
{
	if((args->arg_count < 4) || (args->arg_count > 5)){
                strcpy(message, "ERROR: Wrong arguments! oph_mask_array(input_OPH_TYPE, output_OPH_TYPE, measure_a, measure_b, [missingvalue])");
                return 1;
        }
	...
	int i;
        for(i = 0; i < args->arg_count; i++){
			...
			args->arg_type[i] = REAL_RESULT;
                        ...
                        strcpy(message, "ERROR: Wrong arguments to oph_mask_array function");
                        return 1;
                }
        }
...
void oph_mask_array_deinit(UDF_INIT *initid)
{
