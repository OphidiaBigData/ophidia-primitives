...
int oph_subset_parse(const char* cond, unsigned long len, oph_subset* subset, unsigned long max)
{
	...
	if (!subset->number) return OPH_SUBSET_DATA_ERR;
	...
	while (next && (retval==OPH_SUBSET_OK))
	{
		...
		while (result2 && (retval==OPH_SUBSET_OK))
		{
			switch(number)
			{
				case 0:
					...
					else subset->end[i] = atol(result2);
				...
				case 2:
					...
					if (subset->stride[i]>1) subset->type[i] = OPH_SUBSET_STRIDE;
					break;
				default:
		...
		subset->start[i]--; subset->end[i]--; // Translation to C-like indexing
		subset->count[i] = 1+(subset->end[i]-subset->start[i])/subset->stride[i];
		subset->total += subset->count[i];
		++i;
		if (next) result = strchr(next,OPH_SUBSET_SUBSET_SEPARATOR[0]);
	}

	if (retval != OPH_SUBSET_OK) _oph_subset_free(subset);
...
int oph_subset_vector_free(oph_subset** subset, int number)
{
	...
	{
		int n;
		for (n=0; n<number; ++n) oph_subset_free(subset[n]);
		free(subset);
	}
	return OPH_SUBSET_OK;
}
	...
	*total=0;
	for (j=0; j<size; ++j)
	{
		for (i=0; i<subset->number; ++i)
			if (oph_subset_is_in_subset(oph_subset_id_to_index(j, sizes, size_number), subset->start[i], subset->stride[i], subset->end[i])) break;
		if ((flags[j]=i<subset->number)) (*total)++;
	}
	return OPH_SUBSET_OK;
}
