...
|------------------------------------------------------------------*/
my_bool oph_arg_array_init(UDF_INIT * initid, UDF_ARGS * args, char *message)
{
	if ((args->arg_count < 4) || (args->arg_count > 5)) {
		strcpy(message, "ERROR: Wrong arguments! oph_arg_array(input_OPH_TYPE, output_OPH_TYPE, measure_a, measure_b, [missingvalue])");
		return 1;
	}
	...
	for (i = 0; i < args->arg_count; i++) {
		if (i == 4) {
			...
			strcpy(message, "ERROR: Wrong arguments to oph_arg_array function");
			return 1;
		}
	}
...
void oph_arg_array_deinit(UDF_INIT * initid)
{
...
char *oph_arg_array(UDF_INIT * initid, UDF_ARGS * args, char *result, unsigned long *length, char *is_null, char *error)
{
	int i;
