...
void oph_permute_deinit(UDF_INIT *initid)
{
        ...
        if(initid->ptr)
	{
		...
		if (param->index) { free(param->index); param->index=NULL; }
...
char* oph_permute(UDF_INIT *initid, UDF_ARGS *args, char *result, unsigned long *length, char *is_null, char *error)
{
	...
	else param = (oph_permute_param*)initid->ptr;
	...
	if (!param->error && !param->measure)
	{
		...
		if (args->arg_count > 5) // At least 2 size should be given
		{
			...
			for (i=0; i<number; ++i) if (permutation_indexes[i]!=i) { new_permutation=1; break; }
			...
			if (new_permutation)
			{
				...
				for (id=0; id<measure->numelem; ++id)
				{
					...
					for (i=0; i<number; ++i) new_index[i]=index[number-1-permutation_indexes[number-1-i]]; // Order of sizes in input is reversed
			...
			else pmesg(2, __FILE__, __LINE__, "Same permutation\n");
		}
		else pmesg(2, __FILE__, __LINE__, "No permutation\n");
