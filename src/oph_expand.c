	...
	for (j=js;j<je;++j)
	{
		...
		else
		{
			...
			current = param->start;
	                for (i = 0; i < result->numelem; i++)
			{
				while ((param->index[k] < current-param->step/2.0) && (k<param->num)) k++;
				if (k >= param->num)
				{
...
char* oph_expand(UDF_INIT *initid, UDF_ARGS *args, char *result, unsigned long *length, char *is_null, char *error)
{
	...
	if (args->arg_count>3)
	{
		...
		if (param->index) param->num = args->lengths[3]/sizeof(double);
		else param->num = 0;
		if (args->arg_count>4)
		{
			if (!args->args[4]) param->start = param->index[0];
			else
			{
				param->start = *((double*)args->args[4]);
				if (isnan(param->start)) param->start = param->index[0];
			}
			if (args->arg_count>5)
			{
				if (!args->args[5]) param->step = 1;
				else
				{
					param->step = *((double*)args->args[5]);
					if (isnan(param->step)) param->step = 1;
				}
				if (args->arg_count>6)
				{
					if (!args->args[6]) param->max = param->index[param->num-1];
					else
					{
	...
	if (param->index && param->num) output->numelem = 1 + round((param->max-param->start)/param->step);
	else output->numelem = multim->numelem;
