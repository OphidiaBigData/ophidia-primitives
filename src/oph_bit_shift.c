...
|------------------------------------------------------------------*/
my_bool oph_bit_shift_init(UDF_INIT *initid, UDF_ARGS *args, char *message)
{
        if(args->arg_count < 3 || args->arg_count > 5){
                strcpy(message, "ERROR: Wrong arguments! oph_bit_shift(input_OPH_TYPE, output_OPH_TYPE, bit_measure, [offset], [filling])");
                return 1;
        }
	...
	int i;
        for(i = 0; i < 3; i++){
                if(args->arg_type[i] != STRING_RESULT){
                        strcpy(message, "ERROR: Wrong arguments to oph_bit_shift function");
                        return 1;
                }
        }
...
void oph_bit_shift_deinit(UDF_INIT *initid)
{
...
char* oph_bit_shift(UDF_INIT *initid, UDF_ARGS *args, char *result, unsigned long *length, char *is_null, char *error)
{
        int res = 0;
	oph_bit_param* param;
	...
	if (args->arg_count > 4) filling = *((long long*)args->args[4]) ? OPH_BIT_BLOCK_MAX : 0;

        res = core_oph_bit_shift(param, offset, filling, 0);
        if(res)
	{
