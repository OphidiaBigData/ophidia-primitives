...
void oph_operator_array_deinit(UDF_INIT *initid)
{
...
char* oph_operator_array(UDF_INIT *initid, UDF_ARGS *args, char *result, unsigned long *length, char *is_null, char *error)
{
	int i;
	...
	if (!initid->ptr)
	{
		...
		{
			pmesg(1, __FILE__, __LINE__, "Error in parsing operator name\n");
			*length=0;
			*is_null=0;
			*error=1;
			return NULL;
		}
		char tmp[1+args->lengths[4]];
		strncpy(tmp,args->args[4],args->lengths[4]);
		tmp[args->lengths[4]]=0;
		if (!strcasecmp(tmp,"oph_max")) param->core_oph_oper = core_oph_max_array_multi;
		else if (!strcasecmp(tmp,"oph_min")) param->core_oph_oper = core_oph_min_array_multi;
		else if (!strcasecmp(tmp,"oph_sum")) param->core_oph_oper = core_oph_sum_array_multi;
		else if (!strcasecmp(tmp,"oph_sub")) param->core_oph_oper = core_oph_sub_array_multi;
		else if (!strcasecmp(tmp,"oph_mul")) param->core_oph_oper = core_oph_mul_array_multi;
		else if (!strcasecmp(tmp,"oph_div")) param->core_oph_oper = core_oph_div_array_multi;
		else if (!strcasecmp(tmp,"oph_abs")) param->core_oph_oper = core_oph_abs_array_multi;
		else if (!strcasecmp(tmp,"oph_arg")) param->core_oph_oper = core_oph_arg_array_multi;
		else if (!strcasecmp(tmp,"oph_mask")) param->core_oph_oper = core_oph_mask_array_multi;
		else
		{
