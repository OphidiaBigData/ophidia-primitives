...
char *oph_operator_array(UDF_INIT * initid, UDF_ARGS * args, char *result, unsigned long *length, char *is_null, char *error)
{
	...
	if (!initid->ptr) {
		...
		if (!strcasecmp(tmp, "oph_max"))
			param->core_oph_oper = core_oph_max_array_multi;
		else if (!strcasecmp(tmp, "oph_min"))
			param->core_oph_oper = core_oph_min_array_multi;
		else if (!strcasecmp(tmp, "oph_sum"))
			param->core_oph_oper = core_oph_sum_array_multi;
		else if (!strcasecmp(tmp, "oph_sub"))
			param->core_oph_oper = core_oph_sub_array_multi;
		else if (!strcasecmp(tmp, "oph_mul"))
			param->core_oph_oper = core_oph_mul_array_multi;
		else if (!strcasecmp(tmp, "oph_div"))
			param->core_oph_oper = core_oph_div_array_multi;
		else if (!strcasecmp(tmp, "oph_abs"))
			param->core_oph_oper = core_oph_abs_array_multi;
		else if (!strcasecmp(tmp, "oph_arg"))
			param->core_oph_oper = core_oph_arg_array_multi;
		else if (!strcasecmp(tmp, "oph_mask"))
