		...
		for (i = 0; i < measure->numelem; ++i, elemsize += measure->blocksize) {
			...
			for (j = 0; j < measure->num_measure; ++j) {
				...
				switch (measure->type[j]) {
					case OPH_DOUBLE:
						if (!isnan(*((double *) item)))
							stop = 1;
						else if (missingvalue && (*((double *) item) == (double) *missingvalue))
							stop = 1;
						break;
					case OPH_FLOAT:
						if (!isnan(*((float *) item)))
							stop = 1;
						else if (missingvalue && (*((float *) item) == (float) *missingvalue))
							stop = 1;
						break;
					case OPH_LONG:
						if (missingvalue && (*((long long *) item) == (long long) *missingvalue))
							stop = 1;
						break;
					case OPH_INT:
						if (missingvalue && (*((int *) item) == (int) *missingvalue))
							stop = 1;
						break;
					case OPH_SHORT:
						if (missingvalue && (*((short *) item) == (short) *missingvalue))
							stop = 1;
						break;
					case OPH_BYTE:
						if (missingvalue && (*((char *) item) == (char) *missingvalue))
							stop = 1;
						break;
					default:
