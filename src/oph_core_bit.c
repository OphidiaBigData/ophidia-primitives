...
unsigned long core_oph_bit_length(unsigned long numelem)
{
	...
	if (numelem%OPH_BITSBYTE) result++;
	return result;
}
...
int core_oph_bit_import(oph_bit_param* param)
{
	...
	switch(measure->type)
	{
                case OPH_DOUBLE:
		{
			...
			for (i=0; i<measure->numelem; i++)
			{
				...
				if (*d) tmp2 |= tmp1;
				else tmp2 &= ~tmp1; // bzero(size) could be an alternative
				if (tmp1==1)
				{
					*(param->result+j) = tmp2;
	...
	if (measure->numelem && (tmp1!=OPH_BIT_BLOCK_MSB)) *(param->result+j) = tmp2;
	return 0;
}
...
int core_oph_bit_export(oph_bit_param* param)
{
	...
	switch(measure->type)
	{
                case OPH_DOUBLE:
		{
			for (i=0; i<measure->numelem; i++)
			{
				if (tmp1==OPH_BIT_BLOCK_MSB) tmp2 = *(measure->content+j);
...
int core_oph_bit_operator(oph_bit_param_operator* param)
{
	...
	{
		if (j>=measureA->numelem) A=&_A;
		if (j>=measureB->numelem) B=&_B;
...
int core_oph_bit_not(oph_bit_param* param)
{
	...
	for (j=0; j<param->length; ++j,A++,O++) *O = ~(*A);
	return 0;
}
...
int core_oph_bit_shift(oph_bit_param* param, long long offset, unsigned char filling, char round)
{
	...
	if (offset>0)
	{
		...
		part2 /= OPH_BITSBYTE;
		// pre-shift
		if (part1) memcpy(param->result+part2, measure->content, part1);
		if (part2)
		{
			// rotate
			if (round) memcpy(param->result, measure->content+part1, part2);
			// filling
			else for (i=0,O=(unsigned char*)param->result;i<part2;++i,O++) *O = filling;
		}
		// post-shift
		if (byte_shift)
		{
			O = (unsigned char*)param->result;
			if (round) tmp = *(O+param->length-1) << (OPH_BITSBYTE-byte_shift);
			else tmp = filling << (OPH_BITSBYTE-byte_shift);
	...
	else if (offset<0)
	{
		...
		// pre-shift
		if (part1) memcpy(param->result, measure->content+part2, part1);
		if (part2)
		{
			// rotate
			if (round) memcpy(param->result+part1, measure->content, part2);
			// filling
			else for (i=0,O=(unsigned char*)(param->result+part1);i<part2;++i,O++) *O = filling;
		}
		// post-shift
		if (byte_shift)
		{
			O = (unsigned char*)param->result;
			if (round) tmp = *O >> (OPH_BITSBYTE-byte_shift);
			else tmp = filling >> (OPH_BITSBYTE-byte_shift);
			O += param->length-1;
			for (i=0;i<param->length;++i,O--)
			{
	...
	else memcpy(param->result, measure->content, param->length);
	return 0;
}
...
int core_oph_bit_reverse(oph_bit_param* param)
{
	...
	for (i=0;i<param->length;++i,O++,A--) core_oph_bit_reverse_(A,O); // loop on bytes
	return 0;
}
...
int core_oph_bit_subarray(oph_bit_param_subarray* param)
{
	...
	if (param->step>1)
	{
		for (i=param->start; tot<param->size; tot++, i+=param->step) // loop on bits
		{
			...
			if ((tmp = i%OPH_BITSBYTE)) tmpA>>=tmp;
	...
	else // more optimized for step = 1
	{
		...
		tmpA = OPH_BIT_BLOCK_MSB;
		if ((tmp = param->start%OPH_BITSBYTE)) tmpA>>=tmp;
	...
	if (tmpO<OPH_BIT_BLOCK_MSB) while (tmpO) { *O &= ~tmpO; tmpO>>=1; } // padding
...
int core_oph_bit_aggregate(oph_bit_param_aggregate* param)
{
	...
	if (!param->initialized)
	{
		...
		switch(param->op)
		{
			case OPH_BIT_AND:
			{
				for (i=0; i<param->length; ++i, ++O) *O = OPH_BIT_BLOCK_MAX;
				break;
			}
			case OPH_BIT_OR:
			{
				for (i=0; i<param->length; ++i, ++O) *O = 0;
				break;
			}
			default:
			        pmesg(1, __FILE__, __LINE__, "Operator not recognized or not supported\n");
...
int core_oph_mask(oph_mask_param* param)
{
	...
	if (mask->numelem)
	{
		switch (measure->type)
		{
			case OPH_DOUBLE:
			{
				...
				{
					if ((i>=mask->numelem) || (~(*A) & tmp)) pointer = &param->filling;
					else pointer = (double*)measure->content + i;

					if(core_oph_type_cast(pointer, param->result + i*param->result_elemsize, measure->type, param->result_type))
					{
			...
			case OPH_FLOAT:
			{
				...
				for (i=0;i<measure->numelem;++i) // loop on elements of 'measure' and on 'bits'
				{
					if ((i>=mask->numelem) || (~(*A) & tmp)) { data = (float)param->filling; pointer = &data; }
					else pointer = (float*)measure->content + i;
			...
			case OPH_INT:
			{
				...
				for (i=0;i<measure->numelem;++i) // loop on elements of 'measure' and on 'bits'
				{
					if ((i>=mask->numelem) || (~(*A) & tmp)) { data = (int)param->filling; pointer = &data; }
					else pointer = (int*)measure->content + i;
			...
			case OPH_SHORT:
			{
				...
				for (i=0;i<measure->numelem;++i) // loop on elements of 'measure' and on 'bits'
				{
					if ((i>=mask->numelem) || (~(*A) & tmp)) { data = (short)param->filling; pointer = &data; }
					else pointer = (short*)measure->content + i;
			...
			case OPH_BYTE:
			{
				...
				for (i=0;i<measure->numelem;++i) // loop on elements of 'measure' and on 'bits'
				{
					if ((i>=mask->numelem) || (~(*A) & tmp)) { data = (char)param->filling; pointer = &data; }
					else pointer = (char*)measure->content + i;
			...
			case OPH_LONG:
			{
				...
				for (i=0;i<measure->numelem;++i) // loop on elements of 'measure' and on 'bits'
				{
					if ((i>=mask->numelem) || (~(*A) & tmp)) { data = (long long)param->filling; pointer = &data; }
					else pointer = (long long*)measure->content + i;
	...
	else if (measure->type != param->result_type) // Casting only
	{
		for (i=0;i<measure->numelem;++i)
		{
			if(core_oph_type_cast(measure->content + i*measure->elemsize, param->result + i*param->result_elemsize, measure->type, param->result_type))
			{
				pmesg(1,  __FILE__, __LINE__, "Unable to find result\n");
				return 1;
			}
	...
	else memcpy(param->result, measure->content, param->length); // Identity
