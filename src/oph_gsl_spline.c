...
void oph_gsl_spline_deinit(UDF_INIT *initid)
{
        ...
        if(initid->ptr)
	{
		...
		if (param->extend)
		{
			...
			if (spline)
			{
				if (spline->spline) { gsl_spline_free(spline->spline); spline->spline = NULL; }
	    			if (spline->acc) { gsl_interp_accel_free(spline->acc); spline->acc = NULL; }
				if (spline->tmp) { free(spline->tmp); spline->tmp = NULL; }
			}
			param->extend = NULL;
		}
