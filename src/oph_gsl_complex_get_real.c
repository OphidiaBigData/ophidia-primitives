...
#include "oph_gsl_complex_get_real.h"
...
|------------------------------------------------------------------*/
my_bool oph_gsl_complex_get_real_init(UDF_INIT *initid, UDF_ARGS *args, char *message)
{
    int i = 0;
    /* oph_gsl_complex_get_real(input_OPH_TYPE, output_OPH_TYPE, measure) */
    if(args->arg_count != 3){
        strcpy(message, "ERROR: Wrong arguments! oph_gsl_complex_get_real(input_OPH_TYPE, output_OPH_TYPE, measure)");
        return 1;
    }

    for(i = 0; i < args->arg_count; i++){
            if(args->arg_type[i] != STRING_RESULT){
                    strcpy(message, "ERROR: Wrong arguments to oph_gsl_complex_get_real function");
                    return 1;
            }
    }
...
void oph_gsl_complex_get_real_deinit(UDF_INIT *initid)
{
...
char* oph_gsl_complex_get_real(UDF_INIT *initid, UDF_ARGS *args, char *result, unsigned long *length, char *is_null, char *error)
{
	if (*error)
	{
	        *length=0;
    ...
    for (i = 0; i < measure->num_measure; i++) {
    	...
    	case OPH_COMPLEX_INT:
    		...
    		for (k = 0; k < measure->numelem; k++) {
    			if(core_oph_type_cast(measure->content + (k * measure->blocksize) + run_sum1, output->content + (k * output->blocksize) + run_sum2, OPH_INT, output->type[i], NULL)) {
    				param->error = 1;
    	...
    	case OPH_COMPLEX_LONG:
    		...
    		for (k = 0; k < measure->numelem; k++) {
    			if(core_oph_type_cast(measure->content + (k * measure->blocksize) + run_sum1, output->content + (k * output->blocksize) + run_sum2, OPH_LONG, output->type[i], NULL)) {
    				param->error = 1;
    				pmesg(1,  __FILE__, __LINE__, "Error casting output\n");
    				*length=0;
    				*is_null=0;
    	...
    	case OPH_COMPLEX_FLOAT:
    		...
    		for (k = 0; k < measure->numelem; k++) {
    			if(core_oph_type_cast(measure->content + (k * measure->blocksize) + run_sum1, output->content + (k * output->blocksize) + run_sum2, OPH_FLOAT, output->type[i], NULL)) {
    				param->error = 1;
    				pmesg(1,  __FILE__, __LINE__, "Error casting output\n");
    				*length=0;
    				*is_null=0;
    	...
    	case OPH_COMPLEX_DOUBLE:
    		...
    		for (k = 0; k < measure->numelem; k++) {
    			if(core_oph_type_cast(measure->content + (k * measure->blocksize) + run_sum1, output->content + (k * output->blocksize) + run_sum2, OPH_DOUBLE, output->type[i], NULL)) {
    				param->error = 1;
    				pmesg(1,  __FILE__, __LINE__, "Error casting output\n");
    				*length=0;
    				*is_null=0;
