	...
	for (i = 0; i < measure->num_measure; i++) {
		switch (measure->type[i]) {
			case OPH_COMPLEX_INT:
				...
				for (k = 0; k < measure->numelem; k++) {
					if (core_oph_type_cast
					    (measure->content + (k * measure->blocksize) + run_sum1, output->content + (k * output->blocksize) + run_sum2, OPH_INT, output->type[i], NULL)) {
						param->error = 1;
			...
			case OPH_COMPLEX_LONG:
				...
				for (k = 0; k < measure->numelem; k++) {
					if (core_oph_type_cast
					    (measure->content + (k * measure->blocksize) + run_sum1, output->content + (k * output->blocksize) + run_sum2, OPH_LONG, output->type[i], NULL)) {
						param->error = 1;
						pmesg(1, __FILE__, __LINE__, "Error casting output\n");
						*length = 0;
						*is_null = 0;
			...
			case OPH_COMPLEX_FLOAT:
				...
				for (k = 0; k < measure->numelem; k++) {
					if (core_oph_type_cast
					    (measure->content + (k * measure->blocksize) + run_sum1, output->content + (k * output->blocksize) + run_sum2, OPH_FLOAT, output->type[i], NULL)) {
						param->error = 1;
						pmesg(1, __FILE__, __LINE__, "Error casting output\n");
						*length = 0;
						*is_null = 0;
			...
			case OPH_COMPLEX_DOUBLE:
				...
				for (k = 0; k < measure->numelem; k++) {
					if (core_oph_type_cast
					    (measure->content + (k * measure->blocksize) + run_sum1, output->content + (k * output->blocksize) + run_sum2, OPH_DOUBLE, output->type[i], NULL)) {
						param->error = 1;
						pmesg(1, __FILE__, __LINE__, "Error casting output\n");
						*length = 0;
						*is_null = 0;
