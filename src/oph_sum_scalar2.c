			...
			case OPH_COMPLEX_INT:
				{
					...
					for (i = 0; i < (byte_array->numelem) * 2; i += 2) {
						...
						if (core_oph_type_cast(c.dat, (result->content) + (i * result->elemsize[j]), OPH_DOUBLE, result->type[j]))
							return -1;
						if (core_oph_type_cast(c.dat + 1, (result->content) + ((i + 1) * result->elemsize[j]), OPH_DOUBLE, result->type[j]))
							return -1;
					}
#else
...
|------------------------------------------------------------------*/
my_bool oph_sum_scalar2_init(UDF_INIT * initid, UDF_ARGS * args, char *message)
{
	int i = 0;
	if (args->arg_count < 3 || args->arg_count > 6) {
		strcpy(message, "ERROR: Wrong arguments! oph_sum_scalar2(input_OPH_TYPE, output_OPH_TYPE, measure, [scalar1, scalar2, id_measure])");
		return 1;
	}

	for (i = 0; i < 3; i++) {
		if (args->arg_type[i] != STRING_RESULT) {
			strcpy(message, "ERROR: Wrong arguments to oph_sum_scalar2 function");
			return 1;
		}
	}
...
void oph_sum_scalar2_deinit(UDF_INIT * initid)
{
