...
my_bool oph_reduce3_init(UDF_INIT * initid, UDF_ARGS * args, char *message)
{
	if ((args->arg_count < 3) || (args->arg_count > 9)) {
		strcpy(message,
		       "ERROR: Wrong arguments! oph_reduce3(input_OPH_TYPE, output_OPH_TYPE, measure, [OPH_reduce3_OPERATOR], [binary_count_list], [block_size], [size], [order], [missingvalue])");
		return 1;
	}
...
char *oph_reduce3(UDF_INIT * initid, UDF_ARGS * args, char *result, unsigned long *length, char *is_null, char *error)
{
		...
		if (!hierarchy_set || (hierarchy_set > max_size)) {
			...
			*error = 1;
			...
			for (jj = 1; jj < new_size; ++jj) {
				...
				if (list[jj] > hierarchy_set)
					hierarchy_set = list[jj];
			}
	...
	if (!initid->ptr) {
		...
		tmp = (oph_reduce3_param *) initid->ptr;
