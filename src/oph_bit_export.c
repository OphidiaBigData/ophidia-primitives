...
|------------------------------------------------------------------*/
my_bool oph_bit_export_init(UDF_INIT * initid, UDF_ARGS * args, char *message)
{
	if (args->arg_count != 3) {
		strcpy(message, "ERROR: Wrong arguments! oph_bit_export(input_OPH_TYPE, output_OPH_TYPE, bit_measure)");
		return 1;
	}
	...
	for (i = 0; i < args->arg_count; i++) {
		if (args->arg_type[i] != STRING_RESULT) {
			strcpy(message, "ERROR: Wrong arguments to oph_bit_export function");
			return 1;
		}
	}
...
void oph_bit_export_deinit(UDF_INIT * initid)
{
...
char *oph_bit_export(UDF_INIT * initid, UDF_ARGS * args, char *result, unsigned long *length, char *is_null, char *error)
{
	int res = 0;
	...
	if (!param->measure) {
		...
		//measure->length = &(args->lengths[0]); // Not used
		measure->numelem = core_oph_bit_numelem(args->lengths[2]);	// Const
		param->length = measure->numelem * measure->elemsize;	// bytes
		if (!param->length) {
			*length = 0;
			*is_null = 1;
			*error = 0;
