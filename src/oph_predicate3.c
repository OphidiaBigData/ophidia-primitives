...
int core_oph_predicate3(oph_stringPtr byte_array, char* result)
{
	...
	if (_result->occurrence < 0) occurrence = 1;
	else occurrence = (unsigned long)_result->occurrence;
	switch(byte_array->type)
	{
                case OPH_DOUBLE:
		{
			for (i=_result->occurrence < 0 ? byte_array->numelem-1 : 0; (0<=i) && (i<byte_array->numelem); _result->occurrence < 0 ? --i : ++i)
			{
				...
				{
					occurrence_number++;
					if (occurrence != occurrence_number) res=0;
				}
				temporary = evaluator_evaluate_x (_result->f[res?2:3], *((double*)(byte_array->content+i*byte_array->elemsize)));
		...
		case OPH_FLOAT:
		{
			for (i=_result->occurrence < 0 ? byte_array->numelem-1 : 0; (0<=i) && (i<byte_array->numelem); _result->occurrence < 0 ? --i : ++i)
			{
				...
				temporary = evaluator_evaluate_x (_result->f[res?2:3], *((float*)(byte_array->content+i*byte_array->elemsize)));
				if(core_oph_type_cast(&temporary, (((char*)_result->f[0])+i*_result->result_elemsize), OPH_DOUBLE, _result->result_type, byte_array->missingvalue))
				{
				        pmesg(1,  __FILE__, __LINE__, "Unable to find result\n");
					return 1;
		...
		case OPH_INT:
		{
			for (i=_result->occurrence < 0 ? byte_array->numelem-1 : 0; (0<=i) && (i<byte_array->numelem); _result->occurrence < 0 ? --i : ++i)
			{
				...
				temporary = evaluator_evaluate_x (_result->f[res?2:3], *((int*)(byte_array->content+i*byte_array->elemsize)));
				if(core_oph_type_cast(&temporary, (((char*)_result->f[0])+i*_result->result_elemsize), OPH_DOUBLE, _result->result_type, byte_array->missingvalue))
				{
				        pmesg(1,  __FILE__, __LINE__, "Unable to find result\n");
					return 1;
		...
		case OPH_SHORT:
		{
			for (i=_result->occurrence < 0 ? byte_array->numelem-1 : 0; (0<=i) && (i<byte_array->numelem); _result->occurrence < 0 ? --i : ++i)
			{
				...
				temporary = evaluator_evaluate_x (_result->f[res?2:3], *((short*)(byte_array->content+i*byte_array->elemsize)));
				if(core_oph_type_cast(&temporary, (((char*)_result->f[0])+i*_result->result_elemsize), OPH_DOUBLE, _result->result_type, byte_array->missingvalue))
				{
				        pmesg(1,  __FILE__, __LINE__, "Unable to find result\n");
					return 1;
		...
		case OPH_BYTE:
		{
			for (i=_result->occurrence < 0 ? byte_array->numelem-1 : 0; (0<=i) && (i<byte_array->numelem); _result->occurrence < 0 ? --i : ++i)
			{
				...
				temporary = evaluator_evaluate_x (_result->f[res?2:3], *((char*)(byte_array->content+i*byte_array->elemsize)));
				if(core_oph_type_cast(&temporary, (((char*)_result->f[0])+i*_result->result_elemsize), OPH_DOUBLE, _result->result_type, byte_array->missingvalue))
				{
				        pmesg(1,  __FILE__, __LINE__, "Unable to find result\n");
					return 1;
		...
		case OPH_LONG:
		{
			for (i=_result->occurrence < 0 ? byte_array->numelem-1 : 0; (0<=i) && (i<byte_array->numelem); _result->occurrence < 0 ? --i : ++i)
			{
				...
				temporary = evaluator_evaluate_x (_result->f[res?2:3], *((long long*)(byte_array->content+i*byte_array->elemsize)));
				if(core_oph_type_cast(&temporary, (((char*)_result->f[0])+i*_result->result_elemsize), OPH_DOUBLE, _result->result_type, byte_array->missingvalue))
				{
				        pmesg(1,  __FILE__, __LINE__, "Unable to find result\n");
					return 1;
...
char* oph_predicate3(UDF_INIT *initid, UDF_ARGS *args, char *result, unsigned long *length, char *is_null, char *error)
{
	...
	if(!initid->ptr)
	{
		...
		if (args->arg_count > 6)
		{
			...
			{
				if (!strcasecmp(buffer,OPH_PREDICATE3_FIRST_OCCURRENCE) || !strcasecmp(buffer,OPH_PREDICATE3_BEGIN_OCCURRENCE)) param->occurrence=1;
				else if (!strcasecmp(buffer,OPH_PREDICATE3_LAST_OCCURRENCE) || !strcasecmp(buffer,OPH_PREDICATE3_END_OCCURRENCE)) param->occurrence=-1;
				else
				{
					if (args->arg_type[6] == STRING_RESULT) param->occurrence = (long)strtol(buffer,NULL,10);
					else param->occurrence = *((long long*)args->args[6]);
	...
	else param = (oph_predicate3_param*)initid->ptr;
