	...
	for(j = 0; j < byte_array->num_measure; j++)
	{
        	...
        	switch(byte_array->type[j])
		{
                	case OPH_DOUBLE:
			{
				...
				for (i = 0; i < byte_array->numelem; i++, current+=byte_array->blocksize, d++) memcpy(d, current, sizeof(double));
				...
				for (i = 0; i < byte_array->numelem; i++, current+=byte_array->blocksize, d++) memcpy(d, current, sizeof(float));
			...
			case OPH_LONG:{
				...
				for (i = 0; i < byte_array->numelem; i++, current+=byte_array->blocksize, d++) memcpy(d, current, sizeof(long long));
...
void oph_reduce_deinit(UDF_INIT *initid)
{
        ...
        if(initid->ptr)
	{
		...
		if (multimeasure->content) free(multimeasure->content);
	...
	if(initid->extension){
		if( ((oph_request_multi*)(initid->extension))->measure){
			multimeasure=(oph_multistring*)( ((oph_request_multi*)(initid->extension))->measure);
                	if (multimeasure) free_oph_multistring(multimeasure);
	                multimeasure=NULL;
...
char* oph_reduce(UDF_INIT *initid, UDF_ARGS *args, char *result, unsigned long *length, char *is_null, char *error)
{
	...
	oph_oper myoper = DEFAULT_OPER;
 	...
 	if(!initid->extension){
		...
		((oph_request_multi*)(initid->extension))->core_oph_oper = NULL;
			...
			if (myoper == OPH_QUANTILE) ((oph_request_multi*)(initid->extension))->core_oph_oper = core_oph_quantile_multi;
        	}
	...
	output = (oph_multistring*)((initid->ptr));
        ...
        if(args->arg_count > 4) block_count = *((long long*)(args->args[4]));
	if (!block_count) block_count = multim->numelem;
