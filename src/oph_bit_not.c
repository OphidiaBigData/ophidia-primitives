...
|------------------------------------------------------------------*/
my_bool oph_bit_not_init(UDF_INIT * initid, UDF_ARGS * args, char *message)
{
	if (args->arg_count != 3) {
		strcpy(message, "ERROR: Wrong arguments! oph_bit_not(input_OPH_TYPE, output_OPH_TYPE, bit_measure)");
		return 1;
	}
	...
	for (i = 0; i < args->arg_count; i++) {
		if (args->arg_type[i] != STRING_RESULT) {
			strcpy(message, "ERROR: Wrong arguments to oph_bit_not function");
			return 1;
		}
	}
...
void oph_bit_not_deinit(UDF_INIT * initid)
{
...
char *oph_bit_not(UDF_INIT * initid, UDF_ARGS * args, char *result, unsigned long *length, char *is_null, char *error)
{
	int res = 0;
	oph_bit_param *param;
