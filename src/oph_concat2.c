...
char* oph_concat2(UDF_INIT *initid, UDF_ARGS *args, char *result, unsigned long *length, char *is_null, char *error)
{
	...
	if (!param->error && !param->measure)
	{
		...
		for (i=0;i<args->arg_count-2;++i)
		{
			...
			if (i <= 1) numelem_total += measure[i].numelem; // Almost two measures are considered
			...
			else for (j=0;j<measure->num_measure;++j)
			{
	...
	else
	{
		...
		{
			measure->param++;
			if (measure->param > args->arg_count-3) measure->param = 1;
		}
	}
