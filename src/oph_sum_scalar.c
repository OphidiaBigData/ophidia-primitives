...
my_bool oph_sum_scalar_init(UDF_INIT *initid, UDF_ARGS *args, char *message)
{
        ...
        if(args->arg_count < 3 || args->arg_count > 5){
                strcpy(message, "ERROR: Wrong arguments! oph_sum_scalar(input_OPH_TYPE, output_OPH_TYPE, measure, [scalar, id_measure])");
                return 1;
        }
...
char* oph_sum_scalar(UDF_INIT *initid, UDF_ARGS *args, char *result, unsigned long *length, char *is_null, char *error)
{
	oph_multistring *multim;
        ...
        int id = 0;
	double scalar = 0;
	if(args->arg_count > 3) scalar = *((double*) args->args[3]);
