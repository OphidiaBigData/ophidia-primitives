...
my_bool oph_gsl_idwt_init(UDF_INIT *initid, UDF_ARGS *args, char *message)
{
    ...
    if(args->arg_count < 3 || args->arg_count > 5 ){
            strcpy(message, "ERROR: Wrong arguments! oph_gsl_idwt(input_OPH_TYPE, output_OPH_TYPE, measure,[WAVELET_FAMILY],[WAVELET_MEMBER])");
            return 1;
    }

    for(i = 0; i < 3; i++){
            if(args->arg_type[i] != STRING_RESULT){
                    strcpy(message, "ERROR: Wrong arguments to oph_gsl_idwt function");
                    return 1;
            }
    }
...
char* oph_gsl_idwt(UDF_INIT *initid, UDF_ARGS *args, char *result, unsigned long *length, char *is_null, char *error)
{
    ...
    if (gsl_wavelet_transform_inverse(extra->w,(double *) output->content,1,output->numelem,extra->ws)) {
