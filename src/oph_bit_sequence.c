...
char *oph_bit_sequence(UDF_INIT * initid, UDF_ARGS * args, char *result, unsigned long *length, char *is_null, char *error)
{
	...
	if (args->arg_count > 3) {
		...
		if (mask) {
			mask_length = 0;
			if (args->lengths[3] <= OPH_BIT_SEQUENCE_STATS_NUMBER)
				for (i = 0; i < args->lengths[3]; ++i) {
					if (mask[i] == '1')
						mask_length++;
					else if (mask[i] != '0')
						break;
	...
	if (args->arg_count > 4)
		value = *((long long *) args->args[4]) ? OPH_BIT_BLOCK_MAX : 0;
	...
	*(param->result) = 0;
	...
	if (!mask || (mask[0] == '1')) {
		core_oph_type_cast(&count, param->result + current, OPH_LONG, measure->type, NULL);
		current += measure->elemsize;
	}
	if (mask && (mask[1] == '1')) {
		double mean = (double) sum / (double) count;
		core_oph_type_cast(&mean, param->result + current, OPH_DOUBLE, measure->type, NULL);
		current += measure->elemsize;
	}
	if (mask && (mask[2] == '1')) {
		core_oph_type_cast(&maximum, param->result + current, OPH_LONG, measure->type, NULL);
		current += measure->elemsize;
	}
	if (mask && (mask[3] == '1')) {
		core_oph_type_cast(&minimum, param->result + current, OPH_LONG, measure->type, NULL);
