	...
	switch(byte_array->type)
	{
                case OPH_DOUBLE:
		{
			for (i=_result->occurrence < 0 ? byte_array->numelem-1 : 0; (0<=i) && (i<byte_array->numelem); _result->occurrence < 0 ? --i : ++i)
			{
				...
				{
					occurrence_number++;
					if (occurrence != occurrence_number) r=0;
				}
...
my_bool oph_predicate2_init(UDF_INIT *initid, UDF_ARGS *args, char *message)
{
        if((args->arg_count < 7) || (args->arg_count > 8)){
                strcpy(message, "ERROR: Wrong arguments! oph_predicate2(input_OPH_TYPE, output_OPH_TYPE, measure, checking_expression, comparison_operator, output_expression1, output_expression2, [occurrence])");
                return 1;
        }
...
void oph_predicate2_deinit(UDF_INIT *initid)
{
        ...
        if(initid->ptr)
	{
		...
		for (i=1;i<4;++i) if (((oph_predicate2_param*)initid->ptr)->f[i])
		{
...
char* oph_predicate2(UDF_INIT *initid, UDF_ARGS *args, char *result, unsigned long *length, char *is_null, char *error)
{
	...
	if(!initid->ptr)
	{
		...
		if (args->arg_count > 7)
		{
			...
			{
				if (!strcasecmp(buffer,OPH_PREDICATE2_FIRST_OCCURRENCE) || !strcasecmp(buffer,OPH_PREDICATE2_BEGIN_OCCURRENCE)) param->occurrence=1;
				else if (!strcasecmp(buffer,OPH_PREDICATE2_LAST_OCCURRENCE) || !strcasecmp(buffer,OPH_PREDICATE2_END_OCCURRENCE)) param->occurrence=-1;
				else
				{
					if (args->arg_type[7] == STRING_RESULT) param->occurrence = (long)strtol(buffer,NULL,10);
					else param->occurrence = *((long long*)args->args[7]);
	...
	else param = (oph_predicate2_param*)initid->ptr;
