...
#include "oph_gsl_complex_get_arg.h"
...
|------------------------------------------------------------------*/
my_bool oph_gsl_complex_get_arg_init(UDF_INIT *initid, UDF_ARGS *args, char *message)
{
	int i = 0;
	/* oph_gsl_complex_get_arg(input_OPH_TYPE, output_OPH_TYPE, measure) */
	if (args->arg_count != 3) {
		strcpy(message, "ERROR: Wrong arguments! oph_gsl_complex_get_arg(input_OPH_TYPE, output_OPH_TYPE, measure)");
		return 1;
	}

	for (i = 0; i < args->arg_count; i++) {
		if (args->arg_type[i] != STRING_RESULT) {
			strcpy(message, "ERROR: Wrong arguments to oph_gsl_complex_get_arg function");
			return 1;
		}
	}
...
void oph_gsl_complex_get_arg_deinit(UDF_INIT *initid)
{
...
char *oph_gsl_complex_get_arg(UDF_INIT *initid, UDF_ARGS *args, char *result, unsigned long *length, char *is_null, char *error)
{
	if (*error) {
		*length = 0;
		*is_null = 0;
