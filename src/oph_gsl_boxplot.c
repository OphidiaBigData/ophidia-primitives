...
int core_oph_gsl_boxplot_multi(oph_multistring* byte_array, oph_multistring* result)
{
	...
	if (!output_format)
	{
		...
		{
			if (i) blocksize[i]=blocksize[i-1]; else blocksize[i]=0;
			for (j=0;j<byte_array->num_measure;++j) blocksize[i] += result->elemsize[byte_array->num_measure*i+j];
		}
	}
	for (j=0;j<byte_array->num_measure;++j)
	{
		switch(byte_array->type[j])
		{
			...
			case OPH_INT:{
				...
				for (i=0;i<byte_array->numelem;++i) d[i]=*(int*)(in_string+i*byte_array->blocksize);
			...
			case OPH_SHORT:{
				...
				for (i=0;i<byte_array->numelem;++i) d[i]=*(short*)(in_string+i*byte_array->blocksize);
			...
			case OPH_BYTE:{
				...
				for (i=0;i<byte_array->numelem;++i) d[i]=*(char*)(in_string+i*byte_array->blocksize);
